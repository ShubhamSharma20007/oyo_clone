...
const Singup = () => {
  ...
  const handleChange=(e)=>{
    return setUserValue(prev=>({...prev,[e.target.name]:e.target.value}))
  }
...
//  api called 
  const sendData =async(e)=>{
    ...
    try{
      ...
      if(res.status === 201){
        setUserValue({name:"",email:"",password:""})
  ...
  <div className="  w-1/2">
    <div className="inner w-[80%]">
      ...
      <div className="w-full h-full py-3 px-5 rounded-md bg-white">
        ...
        <input type="text" className='border p-2 w-full my-3 outline-none' placeholder='enter your name' name="name" id="name" value={uservalue.name} onChange={handleChange} required  />
        
        <input type="email" className='border p-2 w-full my-3 outline-none' placeholder='enter your email' name="email" id="email"  value={uservalue.email} onChange={handleChange} required  />
        <input type="password" className='border p-2 w-full my-3 outline-none' placeholder='enter your password' name="password" id="password" value={uservalue.password} onChange={handleChange} required />

        <button type="submit" className='w-2/4 py-3 my-3 rounded-md text-white font-semibold bg-red-500'>Register</button>
         
      <p className='font-semibold py-3 '>Are You Already Resgister ? <Link href={"/login"} className='font-semibold text-red-600'>Click here ?</Link></p>
